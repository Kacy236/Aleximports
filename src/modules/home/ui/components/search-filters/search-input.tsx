...
export const SearchInput = ({
  ...
  return (
    /* CONTAINER CHANGE: 
       Max-width increased to 6xl to accommodate the single line.
       lg:flex-row allows the two rows to merge into one on desktop.
    */
    <div className="flex flex-col lg:flex-row items-center gap-3 w-full max-w-6xl mx-auto p-1">
      ...
      {/* Mobile-only Filter Button (Hidden on Desktop because it's usually in a sidebar) */}
      <Button
        variant="outline"
        className={cn(
      ...
      {/* SECTION 2 & 3: Store Selector & Library 
          On Desktop (lg), these sit right after the search input.
          On Mobile, we wrap them in a div to keep them together if needed.
      */}
        ...
        <DropdownMenu onOpenChange={(open) => !open && setStoreSearch("")}>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className={cn(
                "flex-1 lg:w-60 h-14 justify-between px-4 rounded-xl transition-all bg-white hover:bg-neutral-50",
                boldBorderStyle,
          ...
          >
            <div className="relative mb-2">
              ...
              />
              {storeSearch && (
                <button onClick={() => setStoreSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 p-1">
